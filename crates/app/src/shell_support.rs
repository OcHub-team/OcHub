...
use routedeck_core::paths::get_app_config_dir;
...
pub fn setup_panic_hook() {
    ...
    panic::set_hook(Box::new(move |panic_info| {
        ...
        let system_info =
            panic::catch_unwind(system_info).unwrap_or_else(|_| "Failed to get system info".to_string());
        ...
        if let Ok(mut file) = OpenOptions::new().create(true).append(true).open(&log_path) {
            ...
            let _ = file.flush();
            eprintln!("\n[RouteDeck] Crash log saved to: {}", log_path.display());
        }
        eprintln!("{entry}");
...
fn lock_file_path() -> PathBuf {
    get_app_config_dir().join("routedeck.lock")
}
