//! Control-API routes for the local relay gateway: config, channels, keys,
//! lifecycle, channel health, and one-click app configuration.
...
use ochub_core::gateway::types::{GatewayChannel, GatewayConfig, GatewayKey};
...
async fn running_base_url(s: &ServerState) -> Result<String, AppError> {
    let status = s.app.gateway.status().await;
    if !status.running {
        return Err(AppError::Config(
            "gateway is not running — start it before configuring apps".into(),
        ));
    }
    Ok(status.base_url)
}
...
async fn apply_app(
    ...
    let base_url = running_base_url(&s).await?;
