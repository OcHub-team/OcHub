...
/// 从 user_id 解析 session_id（格式 `..._session_<id>`）。
///
/// Ported from cc-switch `proxy::session::parse_session_from_user_id`
/// (session.rs is otherwise unported); kept local since claude.rs is the
/// only consumer in the transform tier.
