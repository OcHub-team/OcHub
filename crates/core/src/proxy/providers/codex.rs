...
use crate::model::{CodexChatReasoningConfig, Provider};
...
fn extract_codex_base_url_from_toml(config_text: &str) -> Option<String> {
    ...
    crate::apps::codex::extract_codex_base_url(config_text)
}
...
impl CodexAdapter {
    ...
    fn extract_key(&self, provider: &Provider) -> Option<String> {
        ...
        if let Some(auth) = provider.settings_config.get("auth") {
            if let Some(key) = crate::apps::codex::extract_codex_auth_api_key(auth) {
                return Some(key.to_string());
            }
        }
        ...
        if let Some(config) = provider.settings_config.get("config") {
            ...
            if let Some(config_str) = config.as_str() {
                if let Some(key) =
                    crate::apps::codex::extract_codex_experimental_bearer_token(config_str)
                {
                    return Some(key);
                }
            }
