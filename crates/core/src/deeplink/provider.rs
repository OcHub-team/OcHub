...
use crate::model::{ClaudeDesktopMode, Provider, ProviderMeta, UsageScript};
...
const REMOTE_CONFIG_MAX_BYTES: u64 = 256 * 1024;
...
fn fetch_remote_config(config_url: &str) -> Result<String, AppError> {
    ...
    match url.scheme() {
        ...
        scheme => {
            return Err(AppError::InvalidInput(format!(
                "Unsupported configUrl scheme: {scheme}"
            )));
        }
    }
    ...
    let client = reqwest::blocking::Client::builder()
        ...
        .build()
        .map_err(|e| AppError::Message(format!("Failed to build configUrl client: {e}")))?;
    ...
    if response
        .content_length()
        .is_some_and(|len| len > REMOTE_CONFIG_MAX_BYTES)
    {
        return Err(AppError::InvalidInput(format!(
            "configUrl response is too large (max {} KiB)",
            REMOTE_CONFIG_MAX_BYTES / 1024
        )));
    }

    let mut limited = response.take(REMOTE_CONFIG_MAX_BYTES + 1);
    let mut bytes = Vec::new();
    limited
        .read_to_end(&mut bytes)
        .map_err(|e| AppError::Message(format!("Failed to read configUrl response: {e}")))?;
    if bytes.len() as u64 > REMOTE_CONFIG_MAX_BYTES {
        return Err(AppError::InvalidInput(format!(
            "configUrl response is too large (max {} KiB)",
            REMOTE_CONFIG_MAX_BYTES / 1024
        )));
    ...
    String::from_utf8(bytes)
        .map_err(|e| AppError::InvalidInput(format!("Invalid UTF-8 in configUrl response: {e}")))
}
...
fn merge_codex_config(
    ...
    if request.api_key.as_ref().is_none_or(|s| s.is_empty()) {
        ...
        if let Some(api_key) =
            crate::apps::codex::extract_codex_api_key(config.get("auth"), config_str)
        {
