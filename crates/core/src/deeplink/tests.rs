...
// is not a workspace dependency in routedeck-core, so the attribute is dropped;
// `TestHomeGuard` still isolates HOME / CC_SWITCH_TEST_HOME.
#[test]
...
fn test_parse_mcp_apps() {
    ...
    // "gemini" is no longer a valid MCP target app.
