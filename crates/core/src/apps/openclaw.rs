...
pub fn get_openclaw_dir() -> PathBuf {
    ...
    crate::paths::get_home_dir().join(".openclaw")
}
...
fn make_json5_key(key: &str) -> RtJSONValue {
    RtJSONValue::SingleQuotedString(key.to_string())
}

fn json5_key_name(value: &RtJSONValue) -> Option<&str> {
    match value {
        RtJSONValue::SingleQuotedString(s)
        | RtJSONValue::DoubleQuotedString(s)
        | RtJSONValue::Identifier(s) => Some(s.as_str()),
        _ => None,
    }
}
