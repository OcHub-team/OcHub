...
use crate::paths::{atomic_write, get_app_config_dir};
...
pub fn get_hermes_dir() -> PathBuf {
    ...
    crate::paths::get_home_dir().join(".hermes")
}
...
/// `providers:` dict. OcHub treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by OcHub)
...
/// keeping OcHub free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay OcHub must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, an OcHub edit to an unrelated field would silently
...
// OcHub fills that gap by reading/writing the whole file as a markdown
...
mod tests {
    ...
    fn test_guard() -> std::sync::MutexGuard<'static, ()> {
        // Share the crate-wide env lock so HOME-mutating tests in other modules
        // (e.g. deeplink) never run concurrently with these.
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later OcHub edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // an OcHub toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
