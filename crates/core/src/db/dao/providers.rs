...
impl Database {
    pub fn get_all_providers(
        ...
        let provider_iter = stmt
            .query_map(params![app_type], |row| {
                ...
                let sort_index = row.get::<_, Option<i64>>(6)?.map(|v| v as usize);
    ...
    pub fn get_provider_by_id(
        ...
        let result = conn.query_row(
            ...
            |row| {
                ...
                let sort_index = row.get::<_, Option<i64>>(5)?.map(|v| v as usize);
    ...
    pub fn get_current_omo_provider(
        ...
        let row_data: Result<OmoProviderRow, rusqlite::Error> = conn.query_row(
            ...
            |row| {
                Ok((
                    ...
                    row.get(3)?,
                    row.get(4)?,
                    row.get::<_, Option<i64>>(5)?.map(|v| v as usize),
                    row.get(6)?,
                    row.get(7)?,
                ))
            },
        ...
        let meta: crate::model::ProviderMeta = if meta_str.trim().is_empty() {
            crate::model::ProviderMeta::default()
        } else {
    ...
    pub fn has_non_official_seed_provider(&self, app_type: &str) -> Result<bool, AppError> {
        use crate::db::dao::providers_seed::is_official_seed_id;
        let conn = lock_conn!(self.conn);
        let mut stmt = conn
            .prepare("SELECT id FROM providers WHERE app_type = ?1")
            .map_err(|e| AppError::Database(e.to_string()))?;
    ...
    pub fn init_default_official_providers(&self) -> Result<usize, AppError> {
        use crate::db::dao::providers_seed::OFFICIAL_SEEDS;
