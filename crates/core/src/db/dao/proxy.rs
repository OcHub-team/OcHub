...
impl Database {
    ...
    pub async fn get_circuit_breaker_config(
        &self,
    ) -> Result<crate::db::proxy_types::CircuitBreakerConfig, AppError> {
        // 使用 block 限制 conn 的作用域，避免跨 await 持有锁
        let result = {
            let conn = lock_conn!(self.conn);
            conn.query_row(
                ...
                [],
                |row| {
                    Ok(crate::db::proxy_types::CircuitBreakerConfig {
        ...
        match result {
            ...
            Err(rusqlite::Error::QueryReturnedNoRows) => {
                // 如果不存在，初始化默认配置
                self.init_proxy_config_rows().await?;
                Ok(crate::db::proxy_types::CircuitBreakerConfig::default())
            }
            Err(e) => Err(AppError::Database(e.to_string())),
        }
    }
    ...
    pub async fn update_circuit_breaker_config(
        &self,
        config: &crate::db::proxy_types::CircuitBreakerConfig,
    ) -> Result<(), AppError> {
        let conn = lock_conn!(self.conn);
