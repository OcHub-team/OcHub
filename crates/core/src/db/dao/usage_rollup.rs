...
mod tests {
    ...
    // Pre-prune cost backfill is now wired into `Database::rollup_and_prune`
    // (via the ported `backfill_missing_usage_costs_on_conn` in
    // `services/usage_stats.rs`), so this test runs.
    #[test]
