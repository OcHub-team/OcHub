...
impl Database {
    ...
    pub fn export_sql(&self, target_path: &Path) -> Result<(), AppError> {
        ...
        crate::paths::atomic_write(target_path, dump.as_bytes())
    }
    ...
    fn import_sql_string_inner(
        ...
        let sql_content = sql_raw.trim_start_matches('\u{feff}');
        Self::validate_ochub_sql_export(sql_content)?;
    ...
    fn validate_ochub_sql_export(sql: &str) -> Result<(), AppError> {
        let trimmed = sql.trim_start();
        if trimmed.starts_with(OCHUB_SQL_EXPORT_HEADER) {
            return Ok(());
        }
    ...
    /// Manually create a database backup and return its filename.
    pub fn create_backup_file(&self) -> Result<String, AppError> {
