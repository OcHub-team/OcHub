//! Codex MCP 同步和导入模块
//!
//! Ported from cc-switch `src-tauri/src/mcp/codex.rs`.
//!
//! 包含 Codex 的 MCP 配置管理：
...
fn should_sync_codex_mcp() -> bool {
    ...
    crate::apps::codex::get_codex_config_dir().exists()
}
...
pub fn import_from_codex(config: &mut MultiAppConfig) -> Result<usize, AppError> {
    let text = crate::apps::codex::read_and_validate_codex_config_text()?;
...
pub fn sync_enabled_to_codex(config: &MultiAppConfig) -> Result<(), AppError> {
    ...
    let base_text = crate::apps::codex::read_and_validate_codex_config_text()?;
    ...
    let path = crate::apps::codex::get_codex_config_path();
    crate::paths::write_text_file(&path, &new_text)?;
    Ok(())
}
...
pub fn sync_single_server_to_codex(
    ...
    // 读取现有的 config.toml
    let config_path = crate::apps::codex::get_codex_config_path();
    ...
    // 写回文件
    let new_text = doc.to_string();
    crate::paths::write_text_file(&config_path, &new_text)?;
