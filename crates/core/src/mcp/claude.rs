//! Claude MCP 同步和导入模块
//!
//! Ported from cc-switch `src-tauri/src/mcp/claude.rs`.
...
fn should_sync_claude_mcp() -> bool {
    ...
    crate::paths::get_claude_config_dir().exists() || crate::paths::get_claude_mcp_path().exists()
}
...
pub fn sync_enabled_to_claude(config: &MultiAppConfig) -> Result<(), AppError> {
    ...
    claude_mcp::set_mcp_servers_map(&enabled)
}
...
pub fn import_from_claude(config: &mut MultiAppConfig) -> Result<usize, AppError> {
    let text_opt = claude_mcp::read_mcp_json()?;
...
pub fn sync_single_server_to_claude(
    ...
    // 读取现有的 MCP 配置
    let current = claude_mcp::read_mcp_servers_map()?;
    ...
    // 写回
    claude_mcp::set_mcp_servers_map(&updated)
}
...
pub fn remove_server_from_claude(id: &str) -> Result<(), AppError> {
    ...
    // 读取现有的 MCP 配置
    let mut current = claude_mcp::read_mcp_servers_map()?;

    // 移除指定服务器
    current.remove(id);

    // 写回
    claude_mcp::set_mcp_servers_map(&current)
}
