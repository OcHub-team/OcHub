//! Gemini MCP 同步和导入模块
//!
//! Ported from cc-switch `src-tauri/src/mcp/gemini.rs`.
...
pub fn sync_enabled_to_gemini(config: &MultiAppConfig) -> Result<(), AppError> {
    ...
    gemini_mcp::set_mcp_servers_map(&enabled)
}
...
pub fn sync_single_server_to_gemini(
    ...
    // 读取现有的 MCP 配置
    let mut current = gemini_mcp::read_mcp_servers_map()?;

    // 添加/更新当前服务器
    current.insert(id.to_string(), server_spec.clone());

    // 写回
    gemini_mcp::set_mcp_servers_map(&current)
}
