//! oh-my-openagent / oh-my-opencode (OMO) config management.
//! Ported from cc-switch `services/omo.rs`.

use crate::app_state::AppState;
use crate::apps::opencode::get_opencode_dir;
use crate::error::AppError;
use crate::model::Provider;
use crate::paths::{atomic_write, write_json_file};
...
impl OmoService {
    ...
    fn write_profile_config(
        ...
        if let Err(err) = crate::apps::opencode::add_plugin(v.plugin_name) {
    ...
    pub fn delete_config_file(v: &OmoVariant) -> Result<(), AppError> {
        ...
        crate::apps::opencode::remove_plugins_by_prefixes(v.plugin_prefixes)?;
        Ok(())
    }
    ...
    pub fn import_from_local(
        state: &AppState,
        v: &OmoVariant,
    ) -> Result<crate::model::Provider, AppError> {
        ...
        let provider = crate::model::Provider {
            id: provider_id,
            name,
            settings_config,
            website_url: None,
