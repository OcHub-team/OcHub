...
fn read_gemini_credentials_from_file() -> GeminiCredentials {
    let cred_path = crate::apps::gemini::get_gemini_dir().join("oauth_creds.json");
    if !cred_path.exists() {
        return (None, None, CredentialStatus::NotFound, None);
    }
...
///
/// Gemini CLI 的 OAuth Client 凭据（google-gemini/gemini-cli 内置的公开值）不再
/// 硬编码在源码中——本仓库开源，不内置任何凭据。请通过环境变量提供：
/// `GEMINI_OAUTH_CLIENT_ID` 与 `GEMINI_OAUTH_CLIENT_SECRET`。未设置时刷新返回
/// `None`（Gemini 订阅/额度查询不可用，其余功能不受影响）。
