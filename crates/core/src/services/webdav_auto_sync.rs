//! WebDAV auto-sync queue (the `notify_db_changed` consumer).
//!
//! Ported from cc-switch `services/webdav_auto_sync.rs`. Tauri removed: status
...
fn report_auto_sync_status(status: &str, error: Option<&str>) {
    match error {
        Some(message) => {
            log::debug!("[WebDAV][AutoSync] status={status} error={message}")
        }
        None => log::debug!("[WebDAV][AutoSync] status={status}"),
    }
}
