...
The former 0.5.16 claim of complete experimental context management from compaction alone was incorrect. OcHub now forwards all ten History / Notes operations under `/backend-api/codex/alpha/`, including `notes/v2/thread_hint`, to a bound supplier with exactly one enabled Responses channel. It preserves opaque request bodies, encrypted outputs and upstream statuses, replaces credentials, and never retries stateful writes. The distribution server must provide ingestion, storage, retrieval, user ownership checks and persistent upstream-account affinity. Do not change the supplier for existing sessions.

With real ChatGPT login and an OcHub route, context mode **On** explicitly enables `[features.token_budget] enabled = true` and `use_history_notes_extension = true`; real `auth.json` plan claims are not edited. This uses the explicit Codex configuration path rather than the automatic subscription gate. Managed restrictions and extension/model prerequisites still apply. **Auto** remains conservative for suppliers. Remote control retains the official `chatgpt_base_url`. Saving verifies configuration only, not upstream history support or official pairing.
