...
> 宿主适配，不再维护这些模块的源码副本。
>
> **边界修正（2026-08-20）**：`ochub-ui` 只提供通用组件和底层 UI 能力；应用外壳、
> 供应商、网关、设置、主题等产品级页面组合归 OcHub 所有，不再由组件库导出。
...
1. **侧边栏分组**：应用 / 工具（MCP·技能·用量·会话·高级工具）/ **网络（中转）** / 系统（主题·设置）。
...
4. **中转页**：一个卡片代表一套完整商业中转配置；只展示地址、Key、API 格式、模型与思考映射，并提供一键应用到 CLI。端口、渠道、路由和本地服务生命周期全部隐藏。
5. **删除确认**：所有破坏性操作（供应商/MCP/提示词/技能/会话/上游/路由/key/备份/定价）接入 `confirm_dialog`。
