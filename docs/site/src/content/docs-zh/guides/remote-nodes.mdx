...
“远程节点”让一台 OcHub 桌面版控制没有桌面环境的机器。远端只需安装 OpenSSH 和
一个 `ochcli`；AI 编程工具、配置文件和凭据仍然留在远端执行环境中。
...
协商它们已经广告的能力；完整工作区需要新版远端 `ochcli`。对于还不支持受管更新的
旧节点，桌面端会提示先手动升级一次。
...
- 远端安装兼容版本的 `ochcli`；如需受管更新和回滚，请用
  `ochcli node install` 安装。
...
安装更新默认被策略关闭。按后文把 `allowUpdateInstall = true` 打开并重新连接后，一键
更新才会出现。早于受管更新功能的旧节点需要手动升级一次：换成新版 `ochcli`，执行
`ochcli node install`，再重新连接。受管自更新目前支持 macOS 和 Linux（包括 WSL）。
...
默认策略允许读取状态、运行诊断、切换 Provider、管理网关生命周期，以及读取节点更新
状态；远端 Secret 写入、备份恢复和更新安装默认关闭。在远端查看有效策略：
...
| `ochcli: command not found` | 把 `~/.local/bin` 加入非交互 SSH 的 `PATH`，或填写受管 `ochcli` 的绝对路径 |
...
| 更新入口提示手动升级 | 先用新版 `ochcli` 执行一次 `ochcli node install`，启用 `allowUpdateInstall` 后重新连接 |
