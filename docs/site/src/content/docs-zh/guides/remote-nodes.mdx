...
| 工具与更新 | 查看/安装/更新远端 CLI 工具；处理环境变量冲突；维护 Claude、Codex、OpenClaw、Hermes；检查并安装远端 OcHub 更新 |
...
协商它们已经广告的能力；完整工作区需要新版远端 `ochcli`，因此建议桌面端、
`ochcli` 和 `ochubd` 一起升级。

## 使用前提

请准备好：

- 控制端安装 OcHub Desktop `0.4.21` 或更高版本。
...
如果需要从桌面执行恢复数据库、导入 SQL/cc-switch 数据或安装远端更新，请在远端
策略中按需开启高风险能力：
