...
インポート」を選ぶまで書き込みません。インポートしても Claude、Codex、その他のツールへ
自動適用されることはありません。
...
| `dialects` | 必須 | `messages`、`responses`、`chat` の 1 つ以上 |
| `models` | 任意 | この Endpoint で利用できる一意なモデル。最大 500 件 |

OcHub は Endpoint と Dialect の組み合わせごとに Channel を作成します。Manifest は最大
8 Endpoint、100 モデルルールで、文字列 1 つは最大 4 KiB です。同じ Endpoint 内で
Dialect またはモデルが重複すると拒否されます。
