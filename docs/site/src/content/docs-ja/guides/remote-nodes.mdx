...
OcHub Desktop から操作できます。リモート側に必要なのは OpenSSH と 1 つの
`ochcli` だけです。AI コーディングツール、設定、認証情報はリモート環境に残ります。
...
managed updater より前の node には、Desktop が manual upgrade の案内を表示します。
...
- リモート側に互換バージョンの `ochcli`。managed update と rollback を使う場合は
  `ochcli node install` でインストールする。
...
update install は policy で既定無効です。後述の `allowUpdateInstall = true` を設定して
再接続してください。managed updater より前の node は一度だけ手動で current
`ochcli` に置き換え、`ochcli node install` を実行して再接続します。managed
self-update は現在 macOS と Linux（WSL を含む）をサポートします。
...
既定 policy は status、diagnostics、Provider switching、Gateway lifecycle、node update
status の読み取りを許可します。remote secret write、backup restore、update install は
無効です。remote node で確認します。
...
| `ochcli: command not found` | 非対話 SSH の `PATH` に `~/.local/bin` を加えるか、managed `ochcli` の絶対 path を指定する |
...
| update action が manual upgrade を示す | current `ochcli` で一度 `ochcli node install` を実行し、`allowUpdateInstall` を有効にして再接続する |
