...
| Goal | Guide |
| --- | --- |
| Solve real routing needs with aliases, protocol conversion, and exceptions | [Relay routing recipes](/advanced/routing-recipes) |
...
Relay stations offer three modes:
...
Once a tool has been switched onto a station, its provider page can switch the
active upstream without rewriting tool configuration.

Keep a verified backup. When the primary fails:

1. Switch the app to the backup station.
...
Automatic routing selects and retries interfaces inside one station. Moving
between primary and backup station cards is still manual.
...
## Advanced provider editing

### Common configuration

Common configuration is a snippet shared by providers for one app. Enabling
**Apply common configuration to this provider** merges it during writes.

Before extracting from the current config, make sure it contains no key or
model that belongs to only one provider, or that value will spread to every
provider that enables the snippet.
