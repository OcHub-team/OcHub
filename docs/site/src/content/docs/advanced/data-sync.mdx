...
<OcHubStepsVisual
  locale="en"
  labels="Open Database backups|Create a snapshot|Name and verify it|Restore with a safety backup"
...
<OcHubStepsVisual
  locale="en"
  labels="Enter remote details|Test without saving|Save the target|Upload a snapshot|Check last sync"
...
<OcHubStepsVisual
  locale="en"
  labels="Stop edits on other devices|Verify target and profile|Confirm restore|Keep the safety backup|Restart and inspect"
