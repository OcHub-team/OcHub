...
All four tools support provider management, but their write modes and shared
capabilities differ.

| App | Write mode | MCP | Skills | Default directory |
| --- | --- | --- | --- | --- |
| Grok Build | Current-provider switching | Yes | No | `~/.grok` |
| OpenCode | Multiple providers coexist | Yes | Yes | `~/.config/opencode` |
| OpenClaw | Multiple providers coexist | No | No | `~/.openclaw` |
| Hermes | Multiple providers coexist | Yes | Yes | `~/.hermes` |

In coexistence mode, OcHub writes several providers into one tool
configuration. Remove from tool deletes only that provider, not the others.
...
corresponding file and clears current markers on related providers:

1. Read OMO or OMO Slim first.
2. Back up the OpenCode directory.
3. Disable only when it is no longer needed.
4. Return to the OpenCode page and verify the current provider.
...
| --- | --- |
| Provider missing | It was saved but not added, or the directory is wrong |
| One provider replaced another | Duplicate Provider ID in a coexistence app |
...
[Manage providers](/guides/providers).
