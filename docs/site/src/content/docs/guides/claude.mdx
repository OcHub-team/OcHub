...
<OcHubStepsVisual
  locale="en"
  app="Claude Code"
  labels="Open Claude Code and add a connection|Enter the Anthropic-compatible endpoint|Choose the authentication variable|Enter the API key|Start with one model|Review settings.json|Save and switch|Verify from a new terminal"
