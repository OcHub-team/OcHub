...
no desktop environment. The remote machine only needs OpenSSH and one `ochcli`
executable; configuration and AI coding tools continue to run on that machine.
...
`ochcli`. Desktop shows a manual-upgrade hint for a legacy node that predates
the managed updater.
...
- A compatible `ochcli` on the remote machine, installed with `ochcli node
  install` for managed update and rollback.
...
Update installation is disabled by policy until you set
`allowUpdateInstall = true` as described below and reconnect. A node from a
release before managed update support must be upgraded manually once: replace
the old CLI with a current `ochcli`, run `ochcli node install`, and reconnect.
Managed self-update currently supports macOS and Linux, including WSL.
...
switching, and Gateway lifecycle. It also permits reading node update status,
but remote secret writes, backup restore, and update installation remain
disabled. Inspect the effective policy on the
remote node with:
...
| `ochcli: command not found` | Put `~/.local/bin` in the non-interactive SSH `PATH`, or enter the managed `ochcli` absolute path |
...
| Update action says manual upgrade | Install a current `ochcli` once with `ochcli node install`, enable `allowUpdateInstall`, then reconnect |
