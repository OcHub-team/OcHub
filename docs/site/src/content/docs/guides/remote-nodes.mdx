...
| Tools and updates | Inspect/install/update remote CLI tools; repair environment conflicts; maintain Claude, Codex, OpenClaw, and Hermes; check and install remote OcHub updates |
...
`ochcli`. Upgrade Desktop, `ochcli`, and `ochubd` together.

## Requirements

Prepare all of the following:

- OcHub Desktop `0.4.21` or later on the controlling computer.
...
To restore a database, import SQL/cc-switch data, or install an update from the
desktop, explicitly enable the corresponding high-risk capabilities on the
remote node:
...
Writing a new Provider/OpenClaw secret also requires
`allowSecretsWrite = true`. Untouched masked placeholders preserve the remote
