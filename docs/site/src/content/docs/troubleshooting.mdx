...
2. Confirm the current app, provider, or relay station.
...
5. For relay mode, confirm OcHub is running and inspect the Usage request log.
...
- Look for an external-modification conflict or another manager writing the
  file.
- OpenCode, OpenClaw, and Hermes keep multiple providers. Claude, Claude
  Desktop, Codex, and Grok Build emphasize a current provider, so their actions
  differ.

## The address is reachable but requests fail

URL tests and connectivity checks may treat any HTTP response as reachable.
Then check:
...
## Relay station cannot connect

- Confirm the card is enabled.
- Confirm that the station was applied to the app instead of only saved.
- Use connectivity checks to separate local-gateway and upstream failures.
- Temporarily switch to a verified direct connection to prove the CLI still
  works.

If startup reports that the control API port is occupied, close the owning
program and restart, or choose another port with `MS_PORT`. The UI and relay may
still work, while integrations that require the control API do not.
...
- Only relay requests consistently include gateway latency; session imports may
  omit fields.
