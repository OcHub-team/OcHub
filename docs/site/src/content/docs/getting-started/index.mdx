...
should connect **directly to a provider** or go through OcHub's **local relay
station**. On your first run, complete only one of these paths.
...
1. **Review the welcome notice.** Read the configuration-write explanation. If
   OcHub detects cc-switch, it lists the providers, MCP servers, and skill
...
4. **Open an app page.** Under Apps in the sidebar, choose Claude Code, Codex,
   OpenCode, OpenClaw, Hermes, Grok Build, or Claude Desktop.
...
| Relay stations | Manage local-gateway upstreams, model mapping, and fallback |
...
| Share one upstream across multiple tools | [Relay station](/getting-started/relay) |
| Rename models, convert protocols, or switch to a backup upstream | [Relay station](/getting-started/relay) |
...
- [Configure your first relay station](/getting-started/relay)
