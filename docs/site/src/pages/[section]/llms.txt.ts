...
export async function getStaticPaths() {
  ...
  return groups
    ...
    .map((group) => ({
      ...
      props: {
        slug: group.slug,
        // Locale sections are titled in their own language rather than
        // by their slug, so `/zh/llms.txt` opens with "简体中文", not "zh".
        label: localeSectionLabel(group.slug) ?? group.label,
